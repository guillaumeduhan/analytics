...
export function DataTable<T extends Record<string, unknown>>({
  ...
  return (
    <Tabs defaultValue={defaultTab || tabs[0]?.value} className="w-full">
      ...
      {tabs.map((tab) => {
        const tabData = tab.data || defaultData || []
        const tabColumns = tab.columns || defaultColumns || []
        return (
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between px-2 py-1.5 text-xs text-muted-foreground uppercase tracking-wider">
                {tabColumns.map((col) => (
                  <span
                    key={String(col.key)}
                    className={col.key === tabColumns[0].key ? 'flex-1' : 'w-16 text-right'}
                  >
                    {col.header}
                  </span>
                ))}
              ...
              {tabData.length === 0 ? (
                <div className="px-2 py-4 text-sm text-muted-foreground">No data yet</div>
              ) : (
